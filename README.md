...
def export_to_csv(export_rows):
    filename = input("Enter filename (e.g. flights.csv): ").strip()

    with open(filename, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=export_rows[0].keys())
        writer.writeheader()
...
def print_results(results):
    ...
    for result in results:
        ...
        export_rows.append(dict(result))

    export = input("\nWould you like to export this data to a CSV file? (y/n): ").strip().lower()

    if export == "y":
        export_to_csv(export_rows)
